...
impl PidConfig {
    pub fn new(p_gain: f32, i_gain: f32, d_gain: f32, max_output: f32, min_output: f32, max_integral: f32) -> Self {
        Self {
            p_gain,
            i_gain,
            d_gain,
...
impl PID {
    ...
    pub fn compute(&mut self, target : f32, now : f32, delta_time : f32) -> f32 {
        ...
        if self.enable_integral_reset {
            if target > 0.0 && self.previous_target < 0.0 || target < 0.0 && self.previous_target > 0.0 {
                self.integral = 0.0;
            }
        }
        // 前回の目標値を更新します。
        ...
        self.integral = Self::clamp(self.integral, self.config.max_integral, -self.config.max_integral);
        ...
        let output = self.config.p_gain * error + self.config.i_gain * self.integral + self.config.d_gain * derivative;
