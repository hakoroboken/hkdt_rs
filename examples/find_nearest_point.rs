use hkdt_rs::{
    algorithm::{kd_tree, brute_force},
    common::Position2D,
    log_info, log_err
};


const POINT_NUM : usize = 500;
...
fn kd_tree_example(point_cloud : Vec<Position2D>, target : Position2D)
{
    ...
    log_info!("最近傍点座標：\tx={}, y={}", best_p.unwrap().x, best_p.unwrap().y);  
...
fn brute_force_example(point_cloud : Vec<Position2D>, target : Position2D)
{
    ...
    log_info!("探索にかかった時間：\t{}[ns]", elapsed);
    log_info!("最近傍点座標：\tx={}, y={}", point_cloud[best_i].x, point_cloud[best_i].y);  
